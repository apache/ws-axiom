...
public class XMLStreamWriterUtils {
    /**
     * Write base64 encoded data to a stream writer. This will result in one or more {@link
     * javax.xml.stream.XMLStreamConstants#CHARACTERS} events to be written to the stream (or zero
     * events if the data handler produces an empty byte sequence), i.e. the data is streamed from
     * the data handler directly to the stream writer. Since no in-memory base64 representation of
     * the entire binary data is built, this method is suitable for very large amounts of data.
     *
     * <p>Note that this method will always serialize the data as base64 encoded character data.
     * Serialization code should prefer using {@link #writeBlob(XMLStreamWriter, Blob, String,
     * boolean)} or {@link #writeBlob(XMLStreamWriter, BlobProvider, String, boolean)} to enable
     * optimization (if supported by the {@link XMLStreamWriter}).
     *
    ...
    /**
     * Get the {@link BlobWriter} extension for a given {@link XMLStreamWriter}. If the writer
     * exposes the extension, a reference to the extension interface implementation is returned. If
     * the writer doesn't expose the extension, this method returns an instance of the extension
     * interface that emulates the extension (by writing the binary data as base64 character data to
     * the stream).
     *
     * @param writer the stream for which the method should return the {@link BlobWriter} extension
    ...
    /**
     * Write binary content to the stream. Depending on the supplied {@link XMLStreamWriter}, the
     * content will be written as base64 encoded character data or using an optimization scheme such
     * as XOP/MTOM. The method attempts to submit the binary content using the {@link BlobWriter}
     * extension. If the writer doesn't expose this extension, the method will fall back to {@link
     * #writeBase64(XMLStreamWriter, Blob)}.
     *
     * <p>Please refer to the documentation of {@link BlobWriter} for a more detailed description of
     * the semantics of the different arguments.
     *
     * @param writer the stream writer to write the data to
     * @param blob the binary content to write
     * @param contentID an existing content ID for the binary data
     * @param optimize indicates whether the content is eligible for optimization
    ...
    /**
     * Write binary content to the stream. This method is similar to {@link
     * #writeBlob(XMLStreamWriter, Blob, String, boolean)}, but supports deferred loading of the
     * data handler.
     *
     * @param writer the stream writer to write the data to
     ...
     *
     * @param writer the stream writer to write the {@code DOCTYPE} declaration to
