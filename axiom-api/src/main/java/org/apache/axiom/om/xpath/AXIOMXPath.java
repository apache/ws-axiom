...
public class AXIOMXPath extends BaseXPath {
    ...
    /**
     * Construct an XPath expression from a given string and initialize its namespace context based
     * on a given element.
     *
     * @param element The element that determines the namespace context of the XPath expression. See
     *     {@link #addNamespaces(OMElement)} for more details.
     * @param xpathExpr the string representation of the XPath expression.
     * @throws JaxenException if there is a syntax error while parsing the expression or if the
     *     namespace context could not be set up
     */
    ...
    /**
     * Construct an XPath expression from a given attribute. The string representation of the
     * expression is taken from the attribute value, while the attribute's owner element is used to
     * determine the namespace context of the expression.
     *
    ...
    /**
     * Add the namespace declarations of a given {@link OMElement} to the namespace context of an
     * XPath expression. Typically this method is used with an XPath expression appearing in an
     * attribute of the given element.
     *
     * <p>Note that the default namespace is explicitly excluded and not added to the namespace
     * context. This makes the behaviour of this method consistent with the rules followed in XSL
     * stylesheets. Indeed, the XSLT specification defines the namespace context of an XPath
     * expression as follows:
     *
     * <blockquote>
     *
     * the set of namespace declarations are those in scope on the element which has the attribute
     * in which the expression occurs; [...] the default namespace (as declared by xmlns) is not
     * part of this set
     *
     * </blockquote>
     *
