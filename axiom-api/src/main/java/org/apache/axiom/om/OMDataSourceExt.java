 ...
 *   <li>Use {@link OMFactory#createOMElement(OMDataSource, String, OMNamespace)} to build an {@link
 *       OMSourcedElement}.
 *   <li>Add the {@link OMSourcedElement} to the OM tree.
 * </ol>
 *
 * <p>{@code OMDataSourceExt} provides additional methods that are not available on the original
 * {@link OMDataSource}.
 *
 * @see OMDataSource
...
public interface OMDataSourceExt extends OMDataSource {
     ...
     *
     * <p>Data sources that support non destructive read/write should return the object from which
     * the XML is produced. Data sources with destructive read/write should return a non null value
     * only if the backing object has not been consumed yet (even partially).
     *
    ...
    /**
     * Returns true if reading the backing object is destructive. An example of an object with a
     * destructive read is an InputSteam. The owning OMSourcedElement uses this information to
     * detemine if OM tree expansion is needed when reading the OMDataSourceExt.
     *
     * @return boolean
     */
    ...
    /**
     * Returns true if writing the backing object is destructive. An example of an object with a
     * destructive write is an InputStream. The owning OMSourcedElement uses this information to
     * detemine if OM tree expansion is needed when writing the OMDataSourceExt.
     *
     * @return boolean
     */
    ...
    /**
     * Create a copy of the data source. This method is used by {@link
     * OMInformationItem#clone(OMCloneOptions)} when the {@link
     * OMCloneOptions#isCopyOMDataSources()} option is enabled. If the data source is immutable and
     * stateless, then it may return a reference to itself instead of creating a new data source
     * instance.
     *
