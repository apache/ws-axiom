 ...
 * A writable blob.
 *
 * <p>The behavior of the methods defined by this interface is described in terms of four logical
 ...
 * COMMITTED.
 *
 * <p>{@link WritableBlob} instances are generally not thread safe. However, for a blob in state
...
public interface WritableBlob extends Blob {
     ...
     * </ul>
     *
     * <p>The returned stream may implement {@link ReadFromSupport}, especially if the blob stores
     * its data in memory (in which case {@link ReadFromSupport#readFrom(InputStream, long)} would
     * read data directly into the buffers managed by the blob).
     *
     ...
     * implementation may achieve this result in a more efficient way.
     *
     * <p>The blob must be in state NEW when this method is called. It will be in state COMMITTED
     * after this method completes successfully.
     *
     * <p>The method transfers data from the input stream to the blob until the end of the input
     * stream is reached.
     *
