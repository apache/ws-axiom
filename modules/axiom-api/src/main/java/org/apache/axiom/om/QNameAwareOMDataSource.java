...
public interface QNameAwareOMDataSource extends OMDataSource {
     ...
     * 
     * @return the prefix of the element, the empty string if the element has no prefix or
     *         <code>null</code> if the prefix is not known; the implementation is expected to
     *         return an empty string if {@link #getNamespaceURI()} returns an empty string (because
