...
public abstract class BuilderFactory<T extends OMXMLParserWrapper> {
    public static final BuilderFactory<OMXMLParserWrapper> OM = new BuilderFactory<OMXMLParserWrapper>() {
        @Override
        public OMXMLParserWrapper createBuilder(AxiomNodeFactory nodeFactory, BuilderSpec spec) {
            return new OMXMLParserWrapperImpl(
                    new BuilderImpl(spec.getInput(), nodeFactory, PlainXMLModel.INSTANCE, null), spec.getDetachable());
        }
    };

    public static final BuilderFactory<SOAPModelBuilder> SOAP = new BuilderFactory<SOAPModelBuilder>() {
        @Override
            ...
            builder.addListener(new BuilderListener() {
                ...
                public DeferredAction nodeAdded(CoreNode node, int depth) {
                    if (node instanceof AxiomSOAPMessage m) {
                        message = m;
                    } else if (message != null && node instanceof AxiomSOAPEnvelope envelope) {
                        message.initSOAPFactory((SOAPFactory) envelope.getOMFactory());
                    }
                    return null;
                }
            });
